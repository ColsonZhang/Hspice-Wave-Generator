...
class SIGNAL():
    ...
    def updata_signals(self,signals,mode='default'):
...
class MULTI_SIGNAL():
    ...
    def updata_signals(self,signals, mode='default', pulse_en='default', pulse='postive', delay=0, pulse_width=5):
        self.signals = signals
        self.mode = mode
    ...
    def generate_pwl(self):
        ...
        for cnt,instance in enumerate(self.instances):
            instance.updata_signals(self.subsignals[cnt], self.pulse_en)
...
def generate_mult_signal(name, width, signals, mode='default', pulse_en='default', pulse='postive', delay=0, pulse_width=5):
    the_multsignal = MULTI_SIGNAL( name, width )
    the_multsignal.updata_signals(signals, mode, pulse_en=pulse_en, pulse=pulse, delay=delay, pulse_width=pulse_width)
    the_multsignal.generate_pwl()
    return the_multsignal.sources


def generate_single_signal(name, index, signals, mode='default', pulse='postive', delay=0, width=5):
    the_single_signal = SIGNAL(index, name)
    the_single_signal.updata_signals(signals, mode)
